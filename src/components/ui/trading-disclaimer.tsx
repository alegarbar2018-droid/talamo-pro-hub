...
const TradingDisclaimer = ({
  ...
  const getContextualWarning = (context: string) => {
    const warnings = {
      signals: {
        ...
        additionalRisks: ["Las condiciones del mercado pueden cambiar rápidamente invalidando el análisis", "Los precios de ejecución pueden diferir significativamente de los sugeridos", "Cada trader debe considerar su propia tolerancia al riesgo y situación financiera"]
      },
      'copy-trading': {
        ...
        additionalRisks: ["Las estrategias pueden experimentar pérdidas sustanciales inesperadas", "Los resultados mostrados son simulaciones basadas en datos históricos", "Las condiciones de mercado futuras pueden diferir del rendimiento pasado", "Tálamo actúa únicamente como proveedor de señales educativas"]
      },
      academy: {
        ...
        additionalRisks: ["La aplicación práctica requiere experiencia y conocimiento adicional", "Los ejemplos mostrados pueden no reflejar condiciones reales de mercado", "Recomendamos practicar en cuentas demo antes de operar con dinero real"]
      },
      tools: {
        ...
        additionalRisks: ["Los cálculos están basados en datos históricos y modelos teóricos", "Las condiciones reales de mercado pueden diferir significativamente", "Siempre verifique los resultados con su broker antes de operar"]
      },
      general: {
        ...
        additionalRisks: ["Entre el 74-89% de las cuentas de inversores minoristas pierden dinero", "Solo invierta dinero que pueda permitirse perder completamente", "Los CFDs son instrumentos complejos con alto riesgo de pérdidas"]
      }
    };
  ...
  if (variant === 'full') {
    return <Card className={`border-destructive/30 bg-destructive/5 ${className}`}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-muted-foreground ml-4">
                  {warning.additionalRisks.map((risk, index) => <li key={index} className="list-disc">
                      {risk}
                    </li>)}
                </ul>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <Button variant="outline" size="sm" className="border-destructive text-destructive hover:bg-destructive/10" asChild>
                  <a href="https://www.cnmv.es/portal/inversor/CFD.aspx" target="_blank" rel="noopener noreferrer" aria-label="Más información sobre CFDs en CNMV (se abre en ventana nueva)">
                    <ExternalLink className="h-3 w-3 mr-1" />
                    Más info CNMV
                  </a>
                </Button>
                <Button variant="outline" size="sm" className="border-destructive text-destructive hover:bg-destructive/10" asChild>
                  <a href="/legal/risk-disclosure" target="_blank" aria-label="Declaración completa de riesgo (se abre en ventana nueva)">
                    <ExternalLink className="h-3 w-3 mr-1" />
