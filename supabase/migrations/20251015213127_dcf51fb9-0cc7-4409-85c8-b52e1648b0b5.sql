...
-- 2️⃣ Rellenar dedup_key NULL con UUIDs únicos (ANTES de aplicar NOT NULL)
...
-- 6️⃣ Garantizar unicidad del dedup_key
ALTER TABLE IF EXISTS public.signals
...
-- 7️⃣ Crear/actualizar trigger para completar campos faltantes automáticamente
...
DECLARE
  ...
  reward NUMERIC;
BEGIN
  -- Auto-generar dedup_key si no viene (idempotencia garantizada por cliente)
...
COMMENT ON COLUMN public.signals.dedup_key IS 
  'Clave de idempotencia (UUID) desde MT5 EA o sistemas externos. Obligatoria y única.';
...
COMMENT ON FUNCTION public.fill_signal_defaults() IS 
  'Autocompleta campos obligatorios (title, rr, invalidation) antes de INSERT desde MT5 EA';