...
ALTER VIEW public.admin_profile_view SET (security_barrier = true);
...
-- Create a restrictive policy for admin access that logs access attempts
...
USING (
  auth.uid() = user_id OR (
    ...
    -- This policy only allows access to basic fields through the admin view
    -- Sensitive data must be accessed through the secure function
    false -- This effectively blocks direct table access for admins
  )
);
...
-- Add audit trigger for any direct profile table access (as fallback security)
CREATE OR REPLACE FUNCTION public.audit_profile_table_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Log when anyone accesses the profiles table directly (not through user_id match)
  IF auth.uid() IS NOT NULL AND auth.uid() != COALESCE(NEW.user_id, OLD.user_id) THEN
    INSERT INTO public.audit_logs (
      ...
      auth.uid(),
      'profiles.direct_access_attempt',
      'profile',
      jsonb_build_object(
...
CREATE TRIGGER audit_profile_access_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_profile_table_access();